    ...
    namespace gl
    {
        ...
        namespace
        {
            const std::string imageType =
                ...
                "const uint ImageType_RGB_F16_P         = 30;\n";
            ...
            const std::string sampleTexture =
                "// What takes a normalized YUV sample to its 8-bit code value. Video\n"
                "// over eight bits keeps the 8-bit levels in its high byte, so 16-bit\n"
                "// legal white is 235 * 256 of 65535, not 235 / 255 of it (scaling by\n"
                "// 255 left 10-bit white at 253), and chroma zero is 128 * 256 of it.\n"
                ...
                "            float cb = texture(s1, textureCoord).r - 128.0 / codeScale(imageType);\n"
                "            float cr = texture(s2, textureCoord).r - 128.0 / codeScale(imageType);\n"
                ...
                "            float y  = (texture(s0, textureCoord).r * s - 16.0) / (235.0 - 16.0);\n"
                "            float cb = (texture(s1, textureCoord).r * s - 16.0) / (240.0 - 16.0) - 0.5;\n"
                "            float cr = (texture(s2, textureCoord).r * s - 16.0) / (240.0 - 16.0) - 0.5;\n"
                "            c.r = y + (yuvCoefficients.x * cr);\n"
                "            c.g = y - (yuvCoefficients.y * cr) - (yuvCoefficients.z * cb);\n"
                "            c.b = y + (yuvCoefficients.w * cb);\n"
                "        }\n"
                ...
                "            float y  = texture(s0, textureCoord).r;\n"
                "            float cb = texture(s1, textureCoord).r - 128.0 / codeScale(imageType);\n"
                "            float cr = texture(s1, textureCoord).g - 128.0 / codeScale(imageType);\n"
                "            c.r = y + (yuvCoefficients.x * cr);\n"
                "            c.g = y - (yuvCoefficients.y * cr) - (yuvCoefficients.z * cb);\n"
                "            c.b = y + (yuvCoefficients.w * cb);\n"
                "        }\n"
                ...
                "            float y  = (texture(s0, textureCoord).r * s - 16.0) / (235.0 - 16.0);\n"
                "            float cb = (texture(s1, textureCoord).r * s - 16.0) / (240.0 - 16.0) - 0.5;\n"
                "            float cr = (texture(s1, textureCoord).g * s - 16.0) / (240.0 - 16.0) - 0.5;\n"
                "            c.r = y + (yuvCoefficients.x * cr);\n"
                "            c.g = y - (yuvCoefficients.y * cr) - (yuvCoefficients.z * cb);\n"
                "            c.b = y + (yuvCoefficients.w * cb);\n"
                "        }\n"
