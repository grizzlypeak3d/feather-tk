...
namespace ftk
{
    ...
    //! Splitter widget.
    class FTK_UI_API_TYPE Splitter : public IWidget
    {
    protected:
        void _init(
    ...
    public:
        FTK_UI_API virtual ~Splitter();

        //! Create a new widget.
        FTK_UI_API static std::shared_ptr<Splitter> create(
            const std::shared_ptr<Context>&,
            Orientation,
        ...
        //! Get the split value.
        FTK_UI_API float getSplit() const;

        //! Set the split value.
        FTK_UI_API void setSplit(float);
        ...
        FTK_UI_API void setSplitCallback(const std::function<void(float)>&);
