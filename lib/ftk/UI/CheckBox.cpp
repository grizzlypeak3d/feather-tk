...
namespace ftk
{
    ...
    void CheckBox::styleEvent(const StyleEvent& event)
    {
        FTK_P();
        ...
        const DrawEvent& event)
    {
        ...
        if (!p.draw.has_value())
        {
            ...
            p.draw->box = Box2I(
                p.draw->inside.x(),
                p.draw->inside.y() + p.draw->inside.h() / 2 - p.size.checkBox / 2,
                p.size.checkBox,
                p.size.checkBox);
        ...
        event.render->drawText(
            p.draw->glyphs,
            p.size.fontMetrics,
            V2I(p.draw->inside.x() + p.size.checkBox + p.size.spacing + p.size.pad,
