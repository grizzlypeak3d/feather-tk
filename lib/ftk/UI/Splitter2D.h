...
namespace ftk
{
    ...
    class FTK_UI_API_TYPE Splitter2D : public IWidget
    {
    protected:
        void _init(
    ...
    public:
        FTK_UI_API virtual ~Splitter2D();

        //! Create a new widget.
        FTK_UI_API static std::shared_ptr<Splitter2D> create(
            const std::shared_ptr<Context>&,
        ...
        FTK_UI_API void setWidgets(const std::vector<std::shared_ptr<IWidget> >&);

        //! Get where the divisions are, each from zero to one.
        FTK_UI_API const V2F& getSplit() const;

        //! Set where the divisions are.
        FTK_UI_API void setSplit(const V2F&);

        //! Set the callback for a division being dragged.
        FTK_UI_API void setSplitCallback(const std::function<void(const V2F&)>&);
