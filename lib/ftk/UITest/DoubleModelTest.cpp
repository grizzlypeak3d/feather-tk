...
namespace ftk
{
    namespace ui_test
    {
        ...
        void DoubleModelTest::run()
        {
            ...
            model->setValue(0.9);
            FTK_CHECK(0.9 == model->getValue());
            FTK_CHECK(0.9 == value);

            model->setRange(RangeD(0.0, 0.5));
            FTK_CHECK(RangeD(0.0, 0.5) == model->getRange());
            FTK_CHECK(RangeD(0.0, 0.5) == range);
            FTK_CHECK(0.5 == value);

            model->setStep(0.2);
            FTK_CHECK(0.2 == model->getStep());
            model->stepDec();
            FTK_CHECK(fuzzyCompare(0.3, value));
            model->step();
            FTK_CHECK(fuzzyCompare(0.5, value));

            model->setLargeStep(0.3);
            FTK_CHECK(0.3 == model->getLargeStep());
            model->largeStepDec();
            FTK_CHECK(fuzzyCompare(0.2, value));
            model->largeStep();
            FTK_CHECK(fuzzyCompare(0.5, value));

            model->setDefault(0.0);
            FTK_CHECK(0.0 == model->getDefault());
            ...
            // Only setValueSoft() extends a soft range; setValue() clamps
            // so that interactive changes cannot run the range away.
            model->setRange(RangeD(0.0, .5));
            model->setRangeSoft(true);
            FTK_CHECK(model->isRangeSoft());
            model->setValue(2.0);
            FTK_CHECK(.5 == value);
            FTK_CHECK(RangeD(0.0, .5) == range);
            model->setValueSoft(2.0);
            FTK_CHECK(2.0 == value);
            FTK_CHECK(RangeD(0.0, 2.0) == range);
            model->setValueSoft(-1.0);
            FTK_CHECK(-1.0 == value);
            FTK_CHECK(RangeD(-1.0, 2.0) == range);
            model->setRangeSoft(false);
            model->setValueSoft(3.0);
            FTK_CHECK(2.0 == value);
        }
    }
}
